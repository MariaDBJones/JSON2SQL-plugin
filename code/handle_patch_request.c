...
char* handle_patch_request(const char *url) {
...
#endif // PATCHLOG
      char tmp[1]="";
      int length = snprintf(tmp, 1, "UPDATE %s.%s WHERE %s = '%s'", schema, object, column, value);
      query=malloc(length+1);
      snprintf(query, length+1 , "UPDATE %s.%s WHERE %s = '%s'", schema, object, column, value);
#if PATCHLOG == 1
